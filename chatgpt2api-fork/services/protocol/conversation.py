...
def _snapshot_known_ids(
    backend: OpenAIBackendAPI,
    conversation_id: str,
) -> tuple[frozenset[str], frozenset[str]]:
    """Return all file_ids and sediment_ids already in *conversation_id*.

    Called before sending the new generation request so the poll can skip
    pre-existing images and only return the newly generated one.
    Returns empty frozensets when conversation_id is blank or the fetch fails.
    """
    ...
    try:
        ...
        for record in backend._extract_image_tool_records(data):
            file_ids.update(record["file_ids"])
            sediment_ids.update(record["sediment_ids"])
        return frozenset(file_ids), frozenset(sediment_ids)
    except Exception:
        return frozenset(), frozenset()
...
def stream_image_outputs(
    ...
    # Snapshot IDs that already exist so the poll skips old images
    # and waits for only the one generated by this request.
    known_fids, known_sids = _snapshot_known_ids(backend, request.conversation_id)
    last: dict[str, Any] = {}
    for event in conversation_events(
            backend,
            prompt=request.prompt,
    ...
    image_urls = backend.resolve_conversation_image_urls(
        ...
        known_file_ids=known_fids, known_sediment_ids=known_sids)
    if image_urls:
        image_items = [
...
def collect_image_outputs(outputs: Iterable[ImageOutput]) -> dict[str, Any]:
    ...
    for output in outputs:
        ...
        if output.kind == "progress" and output.text:
            ...
            if output.conversation_id:
                upstream_conversation_id = output.conversation_id
    ...
    if upstream_conversation_id:
        result["_upstream_conversation_id"] = upstream_conversation_id
    return result
